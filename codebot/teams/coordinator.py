...
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    if not enable_flag:
        return False
    val = os.environ.get("CODEBOT_COORDINATOR_MODE", "").lower()
...
def match_session_mode(
    ...
    if session_is_coordinator:
        os.environ["CODEBOT_COORDINATOR_MODE"] = "1"
    else:
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    ...
    return """You are CodeBot, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
def get_coordinator_user_context(
    ...
    if worker_tools is None:
        from codebot.agents.tool_filter import IN_PROCESS_TEAMMATE_ALLOWED_TOOLS
