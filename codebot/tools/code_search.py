    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        try:
            ...
            if bm25 and bm25.is_available():
                bm25_results = bm25.search(params.query, top_k=DEFAULT_RECALL)
            ...
            if not fused_ids:
                return ToolResult(
                    output=f'语义搜索未找到相关代码（query: "{params.query}"）。'
                    f"建议用 Grep 工具做关键词搜索。"
                )

            # 6. 取 top_k，组装输出（用向量召回的 payload）
            top_ids = fused_ids[: params.top_k]
            payload_by_id = {h["id"]: h for h in vec_hits}
            lines = [f'语义搜索结果（query: "{params.query}"，融合 {len(fused_ids)} 条召回）:']
