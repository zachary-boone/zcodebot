...
def _session_path(codebot_dir: Path) -> Path:
    return codebot_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(codebot_dir)
...
def load_worktree_session(codebot_dir: Path) -> WorktreeSession | None:
    path = _session_path(codebot_dir)
    if not path.exists():
        return None
    try:
