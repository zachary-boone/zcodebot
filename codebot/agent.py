...
from codebot.client import LLMClient
from codebot.context import (
    CompactBoundary,
...
from codebot.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from codebot.conversation import ThinkingBlock as ConvThinkingBlock
...
from codebot.hooks import HookContext, HookEngine, ToolRejectedError
from codebot.hooks.engine import HookNotification
from codebot.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".codebot" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".codebot" / "plans"
