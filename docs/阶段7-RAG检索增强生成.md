...
LLM 选择器把"找相关记忆"这个**检索问题**，转化成了"让 LLM 看清单选名字"的**生成问题**。慢在三个层面：

**1. 自回归生成（最大头，~2 秒）**

LLM 是自回归模型——生成时一个 token 一个 token 往外吐，**每个 token 都要让整个模型做一次前向计算**（几十亿参数的矩阵乘法），且第 N 个 token 依赖前 N-1 个 token 的结果，**无法并行**：

```
生成 token 1：过一遍整个模型（~30ms）
生成 token 2：过一遍整个模型（~30ms）
...
生成 token 80（一个 50 字 JSON）：~2.4 秒
```

**2. Prefill（处理输入，几百 ms）**

生成前，LLM 要先把输入 prompt（manifest 清单 + query，可能几百到上千 token）过一遍模型做 prefill。几百 token 的 prefill 大概几百毫秒。

**3. 网络往返 + 排队（几百 ms 到不可控）**

- HTTP 请求到 LLM API 服务器：RTT 几十到几百毫秒
- API 服务器繁忙时排队：不可控

**三者叠加**：prefill（几百 ms）+ 自回归生成（2 秒+）+ 网络/排队（几百 ms）= **2-3 秒**。

#### 向量检索为什么快

向量检索把"语义相关性"变成"向量几何距离"——把昂贵的语义理解**从检索时挪到了索引时**：

| 环节 | LLM 选择器 | 向量检索 | 差异原因 |
|------|----------|---------|---------|
| 模型大小 | 几十 GB（LLM） | 几百 MB（embedding 模型） | embedding 模型小 100 倍 |
| 计算类型 | 自回归生成（逐 token，串行） | 一次前向 + 矩阵乘法 | 向量检索不"生成"，只算距离 |
| 检索阶段 | LLM 重新推理语义关系 | 余弦相似度（几百×1536 次乘加） | 纯数学，微秒级 |
| 语义理解时机 | 每次检索都重新推理 | 索引时固化成向量 | 把理解前移到索引阶段 |

**本质区别**：LLM 选择器每次都重新"想"语义关系；向量检索把"想"的结果在索引时存成向量了，检索时只"算"距离。

**一个类比**：
- LLM 选择器 = 找一个懂行的专家，把所有记忆标题念给他听，让他挑——他得听懂（prefill）、思考、再一个个说出来（生成）
- 向量检索 = 给每条记忆和问题都标上 GPS 坐标，算谁离得近——纯几何，不用"思考"

#### LLM 选择器的两阶段设计与 description 短板

LLM 选择器是"先粗筛再精读"的两阶段——**第一阶段只给 LLM 看清单，不是正文全文**：

```
第一阶段（选择）：
  format_memory_manifest() 把所有记忆的 frontmatter 拼成清单文本：
    - [project-scope] [project] /path/auth.md (...): 用户偏好简洁代码风格
    - [user-scope] [feedback] /path/style.md (...): 不准用 type: ignore
    ...
  → LLM 看清单选 ≤5 个文件名（只看 description，不看正文）

第二阶段（读取）：
  render_reminder() 读选中那 5 个文件的正文 → 注入 prompt
```

**为什么不给正文？** 因为记忆可能有几十到几百个，每个正文几百到几千字，全塞进 prompt 远超 context window，且 prefill 几十万 token 要好几秒+巨贵。所以只能给"清单"（每条一句话 description），先筛再读。

**但这恰恰暴露了 LLM 选择器的短板——只看 description，不看正文，会漏召回**：

```
记忆文件 db.md
  frontmatter description: "项目用 PostgreSQL 15"   ← LLM 只看这个
  正文还写了: "连接池用 pgbouncer，端口 6432"         ← LLM 看不到
...
**向量检索能补这个短板**——它对**记忆正文**做 embedding，基于全文语义匹配，不只看 description。上面那个例子，"连接池配置"和 db.md 正文里的"pgbouncer 端口 6432"在向量空间里语义相近，即使 description 没写"连接池"也能命中。

这是"只看 description vs 看正文全文"这个对比项背后的本质差异——不是小差异，是**召回质量的本质差异**。
...
### Q11："为什么用向量检索替代 LLM 选择器？记忆检索这块。"

**深度回答**：两个原因——慢和漏召回。

**慢**：LLM 选择器把检索问题转化成生成问题——让 LLM 看记忆清单选名字。慢在自回归生成（逐 token，每个 token 过整个模型前向，串行无法并行，2 秒+）+ prefill 处理输入 prompt（几百 ms）+ 网络往返。向量检索把"语义相关性"变成"向量几何距离"，embedding 只做一次前向（模型小 100 倍），余弦相似度是矩阵乘法微秒级。本质是"把语义理解从检索时挪到索引时"。

**漏召回**：LLM 选择器是两阶段——先给 LLM 看清单（每条记忆的 frontmatter description，一句话），选完再读正文。但只看 description 会漏——正文里写了"连接池用 pgbouncer"但 description 只写了"用 PostgreSQL 15"，用户问"连接池怎么配"就选不到。向量检索对正文做 embedding，基于全文语义匹配，能补这个短板。

**追问"那为什么不直接给 LLM 看正文？"**：记忆可能几百个，每个正文几百到几千字，全塞进 prompt 远超 context window，prefill 几十万 token 要好几秒+巨贵。所以 LLM 选择器只能给清单先筛——但这恰恰是它的短板。
