...
压缩本身要调用一次 LLM 做摘要，这次调用也可能失败（网络异常、摘要 LLM 超时、prompt too long 等）。如果失败后下一轮又触发压缩、又失败，就会形成"失败循环"——反复消耗 token 和时间却压不下来。

### 5.2 解法：CompactCircuitBreaker

实际代码（`context/manager.py` 第 706-719 行）用的是**失败计数熔断**，不是时间冷却：
...
连续失败 3 次后 `is_open()` 返回 True，`auto_compact` 不再尝试自动压缩，而是返回错误字符串提示用户"自动压缩已熔断（连续失败 3 次），请手动处理或使用 /compact"。

**熔断后不会自动恢复**——必须等用户手动 `/compact`（`manual=True` 绕过 `is_open()` 检查），且压缩成功后 `record_success()` 把计数清零，断路器才解除。

**熔断期间对话还在增长怎么办？** `auto_compact` 返回错误字符串后，agent 会 `yield ErrorEvent` 警告用户，**但不停，继续调主 LLM**。靠两道兜底：① 13K 安全缓冲（触发阈值 = window - 13K，硬限 = window，超阈值≠超硬限，多数情况还能过）；② 真冲破硬限时 LLM API 自身会报 "prompt too long" 拒绝请求。所以不会静默丢数据，最坏是 API 报错让用户看到。

> **注**：早期文档曾把断路器描述成"压缩后 60 秒冷却"（`cooldown_seconds=60` 的 `can_compact()` 时间判断），但代码里没有任何时间冷却逻辑，实际是失败计数熔断。

### 5.3 断路器模式

这是"断路器模式"（Circuit Breaker）的应用——在操作连续失败时主动熔断，防止反复重试浪费资源。类似的例子：
- 电路的保险丝（电流过大时熔断）
- 微服务的断路器（依赖服务连续失败时快速失败，不再重试）
- 数据库连接池（连续连不上时暂停重连）

**核心思想**：连续失败时停止自动重试，把控制权交还给用户（手动 /compact），而不是无限循环失败。
...
防止压缩连续失败时无限重试。如果摘要 LLM 调用连续失败（网络、超时、prompt too long 等），断路器在连续 3 次失败后熔断，`auto_compact` 不再自动尝试，而是提示用户"请手动 /compact"。

**追问：熔断后怎么恢复？**

不会自动恢复。必须用户手动 `/compact`（`manual=True` 绕过熔断检查），且这次压缩成功后 `record_success()` 把 `consecutive_failures` 清零，断路器才解除。这是"把控制权交还用户"的设计——自动机制失灵时让人介入，而不是静默循环失败。

**追问：熔断期间对话还在增长怎么办？**

`auto_compact` 返回错误字符串后，agent 会 `yield ErrorEvent` 警告用户，**但不停，继续调主 LLM**。靠两道兜底：① 13K 安全缓冲（触发阈值 = window - 13K，硬限 = window，超阈值≠超硬限，多数情况还能过）；② 真冲破硬限时 LLM API 自身会报 "prompt too long" 拒绝请求。所以不会静默丢数据，最坏是 API 报错让用户看到。
