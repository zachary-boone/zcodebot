...
### 6.3 RAG 子系统：复用五大扩展机制的应用层能力

RAG（检索增强生成，详见阶段7）不是第六大扩展机制，而是一个**横切多个扩展机制的应用层能力**——它复用了已有的扩展体系，不引入新的扩展维度：

| RAG 组件 | 复用了哪个扩展机制 | 怎么复用 |
|---------|----------------|---------|
| CodeSearch 工具 | **内置工具**（Tool 基类 + 注册表） | 继承 Tool，注册到 `create_default_registry`，和 Grep 并列 |
| 记忆语义检索 | （记忆系统内部增强，非扩展机制） | `recall.py` 加可选参数，向后兼容 |
| ToolSearch 语义增强 | **内置工具**的发现机制 | `search_deferred_semantic`，失败回退关键词 |
| embedding 不可用降级 | **渐进式降级**（项目级哲学，非扩展机制） | `NullEmbedding` + `is_available()` 判断 |

**这体现了正交扩展的威力**——RAG 作为一个新能力，不需要改动五大扩展机制中的任何一个，只是"消费"它们提供的扩展点。加 RAG 没有破坏任何现有扩展机制的正交性。

**面试要点**：当被问"你的 RAG 系统和五大扩展机制是什么关系"时，回答"RAG 是应用层能力，复用了工具扩展机制（CodeSearch 是内置工具）和注册表模式，没有引入新的扩展维度——这正是正交架构的价值，新能力可以平滑接入"。
