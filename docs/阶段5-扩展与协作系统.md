...
- 默认**阻塞**：`await run_to_completion` 等子 Agent 跑完返回结果，自身销毁
- 设 `run_in_background=true` 时**非阻塞**：后台启动，立即返回 task_id，完成后自动通知
...
- `enable_fork=true` 时**强制后台非阻塞**（代码里 `is_background = True`），立即返回 task_id
- `enable_fork=false` 时 fork 被禁用（直接报错），需改用 subagent_type
...
|------|---------|--------|--------|---------|---------|
| SubAgent | 空（全新） | 受限 | 默认阻塞；`run_in_background=true` 时非阻塞 | 返回值 / 自动通知 | 独立子任务 |
| Fork | 复制父 Agent | 受限 | `enable_fork=true` 时强制非阻塞（后台） | 返回值 / 自动通知 | 需要上下文的子任务 |
| Teammate | 空（全新） | 受限 | 非阻塞 | SendMessage | 长期协作 |

> **注**：以上是 `tools/agent_tool.py` 的实际行为。"阻塞"指 `await run_to_completion` 挂起父 Agent 协程，主循环卡住、用户在此期间无法交互；"非阻塞"指 `task_manager.launch()` 起后台任务后立即返回，父 Agent 继续运行。早期文档曾把 Fork 标为"阻塞"，但代码里 `enable_fork=true` 时 `is_background` 被强制设为 True（第 168-169 行），实际是后台非阻塞。
...
**"后端"= Teammate 派生后的运行载体**——队友 agent 以什么形式跑起来：是在主进程里当个协程跑，还是起一个独立的 codebot 进程在终端窗格里跑。三种后端的本质区别是"协程 vs 独立进程"。

速览：

| 后端 | 运行形式 | 平台 |
|------|---------|------|
| `in-process` | 同进程 asyncio 协程 | 全平台 |
| `tmux` | tmux 窗格里的独立 codebot 进程 | 需装 tmux |
| `iterm2` | iTerm2 分屏里的独立 codebot 进程 | 仅 macOS |

#### in-process（进程内）

`teams/spawn_inprocess.py:106`：

```python
task = asyncio.create_task(_run(), name=f"teammate-{name}")
```

队友是**同一个 Python 进程里的一个 asyncio 协程任务**，跑 `agent.run_to_completion`。队友就是个内存里的 Agent 对象，挂在主进程的事件循环上并发执行。没有新进程，没有新终端，用户看不到队友的实时输出（靠 `TeammateProgress` 追踪状态）。

#### tmux

`teams/spawn_tmux.py:71-105`：

```python
pane_id = _run_tmux("split-window", "-h", ...)           # 开一个 tmux 窗格
_run_tmux("send-keys", "-t", pane_id, cli_cmd, "Enter")  # 往窗格发命令并回车
```

`cli_cmd`（`build_cli_command`）长这样：

```
CODEBOT_TEAM_NAME=my-team CODEBOT_TEAMMATE_NAME=worker-1 CODEBOT_MAILBOX_DIR=... \
  codebot -p --work-dir /worktree/xxx --agent-type explore '负责写测试'
```

tmux 后端是**启动一个全新的独立 codebot 子进程**，跑在 tmux 的新窗格里。它有自己的进程空间、自己的工作目录（worktree），通过环境变量 `CODEBOT_TEAM_NAME` / `CODEBOT_TEAMMATE_NAME` / `CODEBOT_MAILBOX_DIR` 知道"我是哪个团队的哪个队友、mailbox 在哪"。用户切换到对应 tmux 窗格就能实时看队友输出。

#### iterm2

`teams/spawn_iterm2.py:53`：

```python
session_id = _run_it2("split-pane", "--command", f"/bin/zsh -c '{cli_cmd}'")
```

和 tmux 本质一样——`it2 split-pane` 在 iTerm2 里开分屏，跑同一个 `cli_cmd`（复用 tmux 的 `build_cli_command`）。只是把"tmux 窗格"换成"iTerm2 分屏"。macOS 专用。

#### 详细对比

| 维度 | in-process | tmux | iterm2 |
|------|-----------|------|--------|
| 运行形式 | asyncio 协程（同进程） | 独立 codebot 子进程 | 独立 codebot 子进程 |
| 进程隔离 | 否（共享进程/事件循环） | 是（独立进程） | 是（独立进程） |
| 工作目录隔离 | 是（各自 worktree） | 是（各自 worktree） | 是（各自 worktree） |
| 能看实时输出 | 否（后台协程，靠 `TeammateProgress`） | 是（切到 tmux 窗格） | 是（切到 iTerm2 分屏） |
| 队友间通信 | 内存对象 + mailbox | mailbox（文件系统，跨进程） | mailbox（文件系统，跨进程） |
| 平台 | 全平台 | 需装 tmux | 仅 macOS（需 iTerm2 + it2） |
| 速度 | 最快（无进程开销） | 稍慢（起进程） | 稍慢（起进程） |

#### 一个澄清：in-process 的"不隔离"指什么

速览表里写 in-process"Agent 间不隔离"略笼统，准确说是**进程/内存层面不隔离**——队友是同进程的协程，一个队友抛未捕获异常可能影响主进程事件循环。但**文件操作层面是隔离的**：`agent_tool.py` 的 `_execute_as_teammate` 里每个队友都调 `worktree_manager.create(...)` 拿到独立 git worktree 作为 `work_dir`，各队友改文件互不踩踏。所以 in-process 的"不隔离"特指进程级，不是文件级。

#### 为什么要有三种

同一个"派生队友"的能力，对应不同的运行载体，权衡四个维度：

- **要速度、不在乎进程隔离**（生产/批处理）→ in-process：协程并发，无进程开销
- **要看队友实时干活、要进程隔离**（调试/演示）→ tmux：每个队友一个窗格，崩了不影响主进程
- **macOS 用户的 tmux 替代** → iterm2：tmux 的 macOS 原生版

这呼应 §6 讲的"正交设计"——**团队协作的能力（派生队友、mailbox 通信、worktree 隔离）和队友的运行载体（后端）是两个正交维度**：后端的选择不改变核心机制，只改变队友"以什么形式存在"。你可以随时切后端而不改团队逻辑。
