...
from codebot.teams.models import (
    AgentTeam,
    BackendType,
    TeammateInfo,
    resolve_team_dir,
...
from codebot.teams.shared_task import SharedTask, SharedTaskStore
from codebot.teams.mailbox import Mailbox, MailboxMessage, create_message
from codebot.teams.registry import AgentNameRegistry
from codebot.teams.backend_detect import BackendDetectionError, detect_backend
from codebot.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from codebot.agents.tool_filter import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("codebot.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".codebot" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("codebot.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_no_backend_raises(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("codebot.teams.backend_detect.shutil.which", return_value=None):
...
class TestCoordinatorMode:
    ...
    def test_enabled_with_flag_and_env(self):
        with patch.dict(os.environ, {"CODEBOT_COORDINATOR_MODE": "1"}):
            assert is_coordinator_mode(enable_flag=True) is True

    def test_flag_without_env(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CODEBOT_COORDINATOR_MODE", None)
    ...
    def test_match_session_mode_switch_to_coordinator(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert os.environ.get("CODEBOT_COORDINATOR_MODE") == "1"
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from codebot.config import AppConfig
    ...
    def test_load_config_with_team_fields(self, tmp_dir):
        from codebot.config import load_config
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from codebot.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from codebot.teams.transcript import load_transcript
        with patch("codebot.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from codebot.prompts import build_system_prompt, BASE_PERSONA
        prompt = build_system_prompt()
        assert BASE_PERSONA in prompt

    def test_coordinator_prompt(self):
        from codebot.prompts import build_system_prompt
    ...
    def test_coordinator_overrides_plan(self):
        from codebot.prompts import build_system_prompt, PLAN_MODE_INSTRUCTIONS
