...
在项目目录下创建 `.codebot/config.yaml`（或全局 `~/.codebot/config.yaml`）：
...
Skills 存放在 `~/.codebot/skills/` 和 `<project>/.codebot/skills/`，支持自定义扩展。
...
Agent 定义存放在 `~/.codebot/agents/` 和 `<project>/.codebot/agents/`。
